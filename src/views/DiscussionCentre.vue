    ...
    <div class="discussionSection">
        ...
        <div class="allMessages">
            ...
            >
                ...
                <div class="deleteButton" v-if="post.userid === id">
                    <button class="btn btn-danger" v-on:click="deletPost(post.id)">Delete</button>
                </div>
            </div>
        </div>
...
export default {
    ...
    created(){
        this.getUserId();
        firebase.firestore().collection('discussionCentre').orderBy('createdAt','desc').onSnapshot(this.getMessages);
    },
    validations: {
        description: {
    ...
    methods:{
        ...
        async getMessages(){
            try{
                this.messages = await PostService.getAllMessages();
            } catch(err){
        ...
        async postMessage(){
            ...
            if(this.$v.$invalid || !this.imageUrlValidation){
                ...
                await PostService.postMessage(data);
                console.log(data)
                this.uploadStatus=false;
                this.comment=""
                this.fileName=''
        ...
        async deletPost(id){
            await PostService.deleteMessage(id)
        }
    }
}
</script>
